...
class ModelIdentification:
    ...
    def model_testing(self):
        ...
        for i in range(len(self.candidates)):
            m, pars = self.candidates[i][0], self.candidates[i][2]
        ...
        if self.verbose:
            print("\n * MODEL TESTING *")
            print(" -> performance:")
            for m, auc, pars in self.candidates:
    ...
    def svm(self):
        for kernel in ['linear', 'poly', 'rbf', 'sigmoid']:
            ...
            self.candidates.append((svc, auc, "kernel={}".format(kernel), False))
    ...
    def nn(self):
        ...
        for size in [50, 100, 200, 500]:
            for act_f in ['logistic', 'tanh', 'relu']:
                for solver in ['lbfgs', 'sgd', 'adam']:
                    ...
                    self.candidates.append((nn, auc, "hidd_lay_sz={}, act_f={}, solver={}".format(size, act_f, solver), False))
        ...
        self.candidates.append((vc, auc, "hidd_lay_sz={}, act_f={}, solver={}".format(*best_conf), False))
    ...
    def display_training_result(model, auc, pars):
        if type(auc) is list:
            print("{} ({})".format(str(type(model)).split('.')[-1][:-2], pars))
