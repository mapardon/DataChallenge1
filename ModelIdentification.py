...
from ModelBase import ModelBase, ExperimentResult


class ExperimentResultMi(ExperimentResult):
...
class ModelIdentification(ModelBase):
    ...
    def __init__(self, train_features: pd.DataFrame, train_labels: pd.DataFrame, test_features: pd.DataFrame,
        ...
        self.candidates: list[ExperimentResultMi] = list()
    ...
    def model_identification(self, model: str, par: str):
    ...
    def gbc(self, par="n_estimators"):
        ...
        if par == "n_estimators":
            for n in [50, 100, 200, 300, 400, 500]:
                ...
                self.candidates.append(ExperimentResultMi("gbc", gbc, auc, "n_estimators", n, False, False))

        elif par == "subsample":
            for s in [0.1, 0.5, 0.75, 0.9, 1.0]:
                gbc = GradientBoostingClassifier(loss="log_loss", subsample=s)
                auc = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ExperimentResultMi("gbc", gbc, auc, "subsample", s, False, False))

        elif par == "min_sample_split":
            for mss in [2, 3, 5, 10]:
                gbc = GradientBoostingClassifier(loss="log_loss", min_samples_split=mss)
                auc = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ExperimentResultMi("gbc", gbc, auc, "min_sample_split", mss, False, False))

        elif par == "max_depth":
            for max_depth in [2, 3, 4, 5, 10, 20]:
                gbc = GradientBoostingClassifier(loss="log_loss", max_depth=max_depth)
                auc = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ExperimentResultMi("gbc", gbc, auc, "max_depth", max_depth, False, False))
        ...
        for e, n in zip(estimators, names):
            if par == "n_estimators_uni":
                for ne in [100, 200, 300, 500][2:]:
                    ...
                    self.candidates.append(ExperimentResultMi("ada", ada, auc, "n_estimators", ne, False, True, n))
        ...
        for e, n in zip(estimators, names):
            if "max_features" in par:
                for mf in [0.5, 0.75, 1.0]:
                    ...
                    self.candidates.append(ExperimentResultMi("bc", bc, auc, "max_features", mf, False, True, n))

            elif par == "n_estimators_uni":
                for ne in [10, 20, 50, 100, 200]:
                    bc = BaggingClassifier(estimator=e, n_estimators=ne)
                    auc = self.parametric_identification_cv(bc, False)
                    self.candidates.append(ExperimentResultMi("bc", bc, auc, "n_estimators", ne, False, True, n))
    ...
    def bc_long(self, par):
        ...
        bc = BaggingClassifier(estimator=e, n_estimators=ne)
        auc = self.parametric_identification_cv(bc, False)
        self.candidates.append(ExperimentResultMi("bc", bc, auc, "n_estimators", ne, False, True, "GBC"))
